...
// Standalone function to read temperature from QMI8658
float QMI8658_readTemp()
{
    // Read the 16-bit temperature value from the sensor
    uint16_t temp_raw = qmi8658_.readWord_reg(Qmi8658Register_Tempearture_L);
    
    // Extract low and high bytes
    uint8_t temp_l = temp_raw & 0xFF;        // Low byte (fractional part)
    uint8_t temp_h = (temp_raw >> 8) & 0xFF; // High byte (integer part)
    
    // Convert according to QMI8658 datasheet:
    // Temperature = high_byte + (low_byte / 256.0)
    // High byte contains integer part, low byte contains fractional part
    float temperature = (float)temp_h + ((float)temp_l / 256.0f);
