#  RP2040 boards (Raspberry Pi Pico, RP2040-Zero) have no auto-reset circuit, so
#  after rp2040load writes the new binary the board occasionally stays parked in
#  the BOOTSEL bootloader instead of starting the app (you'd have to
#  unplug/replug). This post-upload hook kicks it into the application with
#  `picotool reboot -a` -- but ONLY when the board is actually stuck.
#
