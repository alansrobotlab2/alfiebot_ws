...
def launch_setup(context, *args, **kwargs):
    ...
    alfie_launch_prefix = get_package_share_directory("alfie_bringup")

    name = LaunchConfiguration("name").perform(context)
    return [
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                os.path.join(alfie_launch_prefix, "launch", "alfie_camera.launch.py")
            ),
            launch_arguments={
                "name": name,
...
def generate_launch_description():
    ...
    declared_arguments = [
        ...
        DeclareLaunchArgument(
            "params_file",
            default_value=os.path.join(alfie_launch_prefix, "launch", "rgbd.yaml"),
        ),
